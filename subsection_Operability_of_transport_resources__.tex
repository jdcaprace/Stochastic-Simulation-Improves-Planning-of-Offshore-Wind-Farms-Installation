...
Two approaches have been considered in order to analyze the operability of the transport resources. They are differing by the way to use the metocean historical time series in the DES. However, both approaches are using the maximum operational wind speed limit and the maximum operational wave height limit presented in Tab. \ref{tab:ressources}.

\textbf{Deterministic approach}

In the first approach, the operability of the transport resources are assessed in a deterministic way. 

, environmental data are directly used in the DES to assess if an activity can be started based on weather window and workability criterion
